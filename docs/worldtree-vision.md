# The WorldTree: From Logs to Living Knowledge
...
Every completed coordination session writes a structured record into the WorldTree — backed by Postgres and the host filesystem:
...
The most immediate win. Before a new session executes, query the WorldTree for relevant past contributions and inject them as context. Specifics:
...
Today, every session pays full LLM cost. As the WorldTree grows, many session subgoals can be answered without an LLM call at all, or with a much smaller model.

- **Skip the LLM when the WorldTree already answers.** For a new task like "write a Show HN draft for a CLI tool launch," a quick WorldTree search may surface five highly-rated past Show HN drafts from the hackernews-elemental. A small local model can adapt the closest match in milliseconds; the elemental's full LLM call becomes a fallback rather than the default.
...
- **Two-tier inference.** Small local model does retrieval, ranking, and first-draft. Large remote model only invoked for synthesis or when local confidence is low. The WorldTree is what makes the local tier credible.
...
These savings compound. A system processing 100 sessions/day where 60% of subgoals are answered from the WorldTree is paying 40% of the LLM bill it would have paid without one.
...
- **Scored rubrics.** Define rubrics per content type (positioning analysis, HN post, launch report). A critic-elemental scores past contributions against the rubric, storing scores in `session_contributions.metadata`. The WorldTree becomes labeled.
- **Outcome attachment.** When external outcomes are knowable (a HN post's score, a launch's stargazers), attach them to the session. The WorldTree becomes labeled with ground truth, not just rubric scores.
...
- **Negative feedback.** Failed or low-scoring sessions are equally valuable. The WorldTree should preserve them, marked. Future plans can explicitly exclude patterns associated with low-scoring outcomes.
...
The WorldTree is queryable; an MCP-speaking agent should be able to converse with it. This turns the system from an action-execution platform into an exploration partner.

Concrete MCP surface:

**Resources** (read-only, addressable by URI):
- `worldtree://sessions/{sessionId}` → the session manifest
- `worldtree://sessions/{sessionId}/{mode}` → a specific published artifact
- `worldtree://agents/{agentId}/contributions?since=...` → an agent's contribution stream
- `worldtree://realms/{realmId}/sessions` → all sessions in a realm
...
An MCP client connected to this surface is doing interactive WorldTree archaeology. It can be a chat UI, a CLI, another agent — anything that speaks MCP. The WorldTree stops being a passive log and becomes an active conversational partner.
...
- Stand up an MCP server that exposes `worldtree://` resources and the basic query tools listed above.
- Backed entirely by existing tables, no embeddings, no new ML.
- Outcome: any MCP client can browse and query the WorldTree interactively.
...
- CoordinationService consults the WorldTree before planning and before each delegation.
...
**Phase D — Scoring and labeled data**
...
A few non-negotiables that the WorldTree should respect:
...
- **Local-first by default.** Embeddings, search, scoring, retrieval all run inside the deployment. The WorldTree is too valuable to scatter across third-party services.
- **Sessions stay isolated.** Session isolation (per the concurrent-session constitution) means cross-session retrieval is *read* against the WorldTree, never mutation of a sibling session.
- **Cheap to query, expensive to ignore.** Index aggressively. The whole value proposition is fast retrieval; if a query is slow, the WorldTree stops being a planning aid and becomes a curiosity.

## Open questions for discussion

- **Cross-session visibility for private namespaces.** The WorldTree distinguishes `worldtree://public/...` from `worldtree://private/{agentId}/...`. How should query-time visibility respect that boundary?
- **Retention vs. analytics.** Contributions are currently retained indefinitely. As the WorldTree grows large, do we tier (hot/warm/cold) or shard by realm?
- **Externally-attached outcomes.** What's the smallest API that lets a user say "session X led to outcome Y" without coupling the WorldTree to any specific external system?
...
- **Versioning the schema for `dataset` mode.** As we add fields, downstream consumers (fine-tuners, analytics) pin to a schema version. How do we evolve the dataset shape without breaking existing consumers?

These are not blockers — they are the conversations that get easier to have once a meaningful WorldTree actually exists. That part is now true.
