...
class PolicyEngine {
     ...
     */
    async assignRole(subjectType, subjectId, roleId, options) {
     ...
     */
    async revokeRole(subjectType, subjectId, roleId) {
        const subjectKey = `${subjectType}:${subjectId}`;
        const assignments = this.roleAssignments.get(subjectKey) || [];
    ...
    async checkAccess(context) {
        ...
        try {
            ...
            const effectivePermissions = await this.getEffectivePermissions(context.subjectType, context.subjectId);
     ...
     */
    async processApproval(approvalId, decision, approver, reason) {
     ...
     */
    async getEffectivePermissions(subjectType, subjectId) {
     ...
     */
    async getSubjectRoles(subjectType, subjectId) {
        const subjectKey = `${subjectType}:${subjectId}`;
        const assignments = this.roleAssignments.get(subjectKey) || [];
        const roles = [];
        for (const assignment of assignments) {
    ...
    async evaluatePermissions(permissions, context) {
        let bestMatch = null;
        const violations = [];
    ...
    async evaluatePermission(permission, context) {
        ...
        if (permission.scope.resources) {
            const resourceMatch = permission.scope.resources.some(resource => resource.type === context.resourceType &&
    ...
    async evaluateConstraints(constraints, context) {
        if (!constraints) {
    ...
    async getInheritedPermissions(role) {
        ...
        const collectPermissions = async (roleId) => {
    ...
    async recordPolicyViolations(violations, context) {
        const now = Date.now().toString();
    ...
    compareSecurityLevels(level1, level2) {
